...
const MyPostsPage = () => {
  ...
  useEffect(() => {
    const fetchMyPosts = async () => {
      try {
        ...
        console.error("خطأ في جلب منشوراتي:", err.message);
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <section className="bg-gray-100 min-h-screen" dir="rtl">
            ...
            {myPosts.map((post) => (
              ...
              >
                <div className="rounded-lg p-4 shadow-2xl bg-white h-full flex flex-col">
                  ...
                  <h3 className="text-xl font-semibold mb-2 text-right">{post.title}</h3>
                  <div className="mt-auto flex space-x-4 justify-end">
                    <button
